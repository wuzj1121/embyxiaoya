...
if [ $1 ]; then
	...
	if [ $2 ]; then
        	docker run -it --security-opt seccomp=unconfined --rm --net=host -v $1:/media -v $2:/etc/xiaoya -e LANG=C.UTF-8 -e PGID=100 -e PUID=1000 xiaoyaliu/glue:latest /update_all.sh
	...
	case $cpu_arch in
		"x86_64" | *"amd64"*)
			docker run -d --name emby -v $1/config:/config -v $1/xiaoya:/media --net=host --user 1000:100 --restart always emby/embyserver:4.8.0.56
			...
			;;
		"aarch64" | *"arm64"* | *"armv8"* | *"arm/v8"*)
		        docker run -d --name emby -v $1/config:/config -v $1/xiaoya:/media --net=host  --user 1000:100 --restart always emby/embyserver_arm64v8:4.8.0.56
			#docker cp /tmp/Emby.Server.Implementations.dll emby:/system/
                        #docker exec -i emby chmod 644 /system/Emby.Server.Implementations.dll
                        #docker restart emby
			echo "一键全家桶全部安装完成"
